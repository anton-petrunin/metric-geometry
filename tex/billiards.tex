...
Let $\spc{W}$ be a proper length space that has two closed 
...
if $\spc{R}^0\cup\spc{R}^N$, the union of its lower and upper levels forms a convex set in~$\spc{R}$;
...
The end-to-end convexity of $\ddot{\spc{R}}$
...
The collision theorem was proved by Dmitri Burago, Serge Ferleger, and Alexey Kononenko \cite{burago-ferleger-kononenko-1997};
...
It was shown by Dmitri Burago and Sergei Ivanov \cite{burago-ivanov} that the number of collisions that may occur between $n$ identical balls in $\RR^3$ grows at least exponentially in $n$; the two-dimensional case is open so far.
