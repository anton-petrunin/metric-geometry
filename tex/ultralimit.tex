...
For example if $\omega$ is the principal ultrafilter such that $\omega(\{n\})=1$ for some $n\in\NN$, then
$x_\omega=x_n$.
...
Use ultralimits to give a shorter solution of \ref{prop:eps-isometry=isometry}.
\end{thm}
...
Conclude that $(\spc{X}^\omega)^\omega= \spc{X}^\omega$ if and only if $\spc{X}$ compact.
\end{subthm}
