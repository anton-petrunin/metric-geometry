\chapter{Ultralimits}\label{chap:ultralimits}
...
on $\NN$ 
is called an \index{ultrafilter}\emph{ultrafilter} if it meets the following condition:
\begin{subthm}{}
$\omega(\NN)=1$ and 
...
More commonly, a nonprincipal ultrafilter is defined as a collection, say $\mathfrak{F}$, of subsets in $\NN$ such that
\begin{enumerate}
...
Let us define the \index{$\omega$-limit}\emph{$\omega$-limit} of a sequence $x_1,x_2,\dots$ as the point $x_\omega\in\spc{X}$ 
...
Then $x_\omega$ is a \index{partial limit}\emph{partial limit} of $x_n$;
...
In this section, we illustrate the power of ultralimits by proving the following simple claim.
...
The obtained metric component $\spc{Z}=(\spc{X}_\omega)_{p_\omega}$ with marked $p_\omega$ is called the \index{pointed ultralimi}\emph{pointed $\omega$-limit} of $(\spc{X}_n,p_n)$.
...
if needed, we may call it the \index{small ultrapower}\emph{small ultrapower}, and the whole space $\spc{X}^\omega$ could be called the \index{big ultrapower}\emph{big ultrapower} of $\spc{X}$.
...
Let $\spc{X}$ be an infinite countable set with \index{discrete metric}\emph{discrete metric};
...
A metric space $\spc{X}$ is called \index{$n$-point homogeneous}\emph{$n$-point homogeneous} if every distance-preserving map $S\to \spc{X}$ from a subset $S\subset\spc{X}$ with at most $n$ points extends to a global isometry of $\spc{X}$.

}

\begin{thm}{Exercise}\label{ex:two-point-homogeneous}
Show that a small ultrapower of a two-point homogeneous length space is two-point homogeneous.
\end{thm}
