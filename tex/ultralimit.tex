...
Let us define the \emph{$\omega$-limit}\index{$\omega$!$\omega$-limit} of $(x_n)$ as the point $x_\omega$ 
...
Assume $\spc{X}$ is compact, in this case the map $\NN\to\spc{X}$ can be extended to a continuous map from the Stone--\v{C}ech compactification $\beta\NN$ of $\NN$.
Then $x_\omega$ can be regarded as the image of $\omega$.
...
\parit{Proof.} Assume that $(x_n)$ is a Cauchy sequence. 
...
The space $\spc{X}_\omega$ is called \emph{$\omega$-limit}\index{$\omega$-limit space} of $\spc{X}_n$.
