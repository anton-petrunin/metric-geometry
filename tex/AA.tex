A metric space $\spc{X}$ has \index{extendable geodesics}\emph{extendable geodesics} if any two points $x,y\in\spc{X}$
lie on a geodesic $\gamma\:\RR\to \spc{X}$.
We say that a metric space $\spc{X}$ has \index{nonbifurcating geodesics}\emph{nonbifurcating geodesics} if the following condition holds
\textit{if two geodesics in $\spc{X}$ a nontrivial common arc, then their union is a geodesic}.
