...
Then $\width_n\spc{X}<R$ if and only if there is a finite $n$-dimensional somplicial complex $\spc{N}$ and a continuous map $\psi\:\spc{X}\to \spc{N}$
...
\section{Volume profile bound width}
...
Note that Riemannian polyhedron is connected by definition and if $\spc{P}$ is 1-dimensional, then 
...
An $(n-1)$-dimensional subpolyhedron $\spc{Q}\subset\spc{P}$ is called $R$-separating if $\rad U<R$ for each connected component $U$ of the complement $\spc{P}\backslash \spc{Q}$.
...
We apply induction on the dimension $n=\dim\spc{P}$ to show that one can take $c_n=2^n$;
...
\[2^n\cdot \sqrt[n]{\VolPro\spc{P}(r)}< r\]
for some $r>0$.
Fix small $\eps>0$.
Applying \ref{lem:separating} with $r_0=\tfrac12\cdot r$ and $r_1=r$, we have an $r$-separating subpolhedron $\spc{Q}$ in $\spc{P}$ such that 
\begin{align*}
\VolPro_\spc{Q}(r_0) &< \tfrac 2r\cdot \VolPro_\spc{P}(r)+\eps<
\\
&<\frac {2} {r}\cdot\left(\frac{r}{2^n}\right)^n=
\\
&=\left(\frac{r_0}{2^{n-1}}\right)^{n-1};
\end{align*}
that is, $2^{n-1}\cdot \sqrt[n-1]{\VolPro\spc{Q}(r_0)}< r_0$.
Since $\dim\spc{Q}=n-1$ and $c_{n-1}\z=2^{n-1}$, the induction hypothesis implies that
...
The presented proof is a result of a sequence of simplifications and improvements given by Larry Guth, Panos Papasoglu, and Alexander Nabutovsky \cite{guth, papasoglu, nabutovsky}.
In \cite{nabutovsky} the calculations were optimized which gave a better constant in  \ref{thm:width<volpro}:
\[c_n=\sqrt[n]{n!}= \tfrac ne+o(n).\]

The technique for all these proofs have origin in the theory of minimal surfaces.
