...
Denote by $c_0$ the space of all real sequences converging to zero equipped with the sup-norm.
Consider the unit ball $B$ in $c_0$;
denote by $\rho_0$ the metric on $B$.

Consider another length metric $\rho_1$ on $B$ which is different from $\rho_0$ by the conformal factor
...
\parbf{\ref{ex:gross}.}
Suppose such a number does not exist.
