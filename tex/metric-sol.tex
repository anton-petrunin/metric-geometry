...
\parit{Comment.}
A slightly more general version of this statement was proved by Ivar Ekeland \cite{ekeland}.
...
Let $r_0$ be the least upper bound for radii $r>0$ such that the closed ball $\cBall[p,r]$ is compact.
Suppose $\cBall[p,r_0]$ is not compact.
Choose a sequence of points $x_n\in\cBall[p,r_0]$ that has no converging subsequence.
Consider geodesic paths $\gamma_n$ from $p$ to $x_n$.
Pass to a subsequence of $x_n$ so that $\gamma_n(t)$ converges for any $t\in[0,1)$;
let $\gamma_\infty\:[0,1)\to \spc{X}$ be the limit curve.
By the assumption, $\gamma_\infty$ can be are extended to $[0,1]$.
Show that $x_\infty=\gamma_\infty(1)$ is a limit of $x_n$ and arrive at a contradiction.

Further argue as in the proof of the Hopf--Rinow theorem.

\parit{Source:} Communicated by Nina Lebedeva.
