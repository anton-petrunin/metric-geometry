...
a \index{partition of unity}\emph{partition of unity subordinate to the open covering} $\{V_i\}$.
...
Note that the nerve $\spc{N}$ has dimension at most $n$ if and only if the covering $\{V_1,\dots,V_k\}$ has \emph{multiplicity} is at most $n+1$;
...
Recall that star $\Star_{v_i}$ of a vertex $v_i$ is defined as the union of the interiors of all simplicies that have $v_i$ as a vertex.
...
\item 
Usually width is defined using diameter instead of radius, but the results differ at most twice.
...
A polyhedron $\spc{Q}$ is called \emph{pure}; that is, any simplex of $\spc{Q}$ lies in a simplex of maximal dimension $n=\dim \spc{Q}$.


\begin{thm}{Exercise}
Let $\spc{P}$ be an $n$-dimensional Riemannian polyhedron and $\spc{Q}$ be a maximal pure subpolyhedron of  $\spc{P}$.
Show that 
\[\width\spc{Q}=\width\spc{P}.\]
...
Then for any $\delta>0$ there is a \emph{smooth} 1-Lipschitz function $\tilde f\:\spc{P}\to \RR$;
that is, $f$ is smooth on each simplex of the triangulation and $\delta$-close to $f$.
\end{thm}
...
Let $\spc{P}$ be an $n$-dimensional Reimannian polyhedron and $f\:\spc{P}\to \RR$ be a smooth function.
...
Let $\spc{P}$ be an $n$-dimensional Reimannian polyhedron and $f\:\spc{P}\to \RR$ be a smooth 1-Lipschitz function.
...
The idea of the proof is borrowed from the theory of minimal surfaces.
...
Applying the smoothing procedure (\ref{smoothing-procedure}), we can exchange each distance function $\distfun_p$ on $\spc{P}$ by $\delta$-close smooth 1-Lipschitz function, which will be denoted by $\widetilde \distfun_p$.

By Sard's theorem (\ref{sard}), almost all level sets 
...
\end{align*}
we for some $c\z\in(r_0\z+\delta, r_1-\delta)$;
moreover we can assume that the level set $\tilde S_c(p)$ is a subpolyhedron.
...
Note that cutting from $\spc{Q}$ everything inside $\tilde S_c(p)$ and adding $\tilde S_c(p)$ keeps it as a $R$-separating subpolyhedron.
...
Suppose $\spc{M}$ is a compact aspherical $n$-dimensional Riemnnian manifold.
Then 
...
Suppose that $\psi\:\spc{M}\to \spc{N}$ is provided by \ref{prop:space->nerve}.
...
Suppose that $\spc{M}$ is a closed $n$-dimensional Reimannian manifold with \emph{injectivity radius} at least $r$; that is, if $\dist{p}{q}{\spc{M}}<r$, then a minimizing geodesic $[pq]_{\spc{M}}$ is uniquely defined.
Show that
...
Show that the product of nonessential closed manifold with any closed manifold is not essential.
\end{thm}
...
$c_n=\sqrt[n]{n!/2}= \tfrac ne+o(n)$ instead of $n$ in \ref{thm:width<vol}.
