...
Note that the nerve $\spc{N}$ has dimension at most $n$ if and only if the covering $\{V_1,\dots,V_k\}$ has \index{multiplicity of covering}\emph{multiplicity} is at most $n+1$;
...
a \index{triangulation of Riemannian polyhedron}\emph{triangulation} of Riemannian polyhedron  will always be assumed to have the above property on the metric tenor.
...
\begin{thm}{Coarea inequality}\index{coarea inequality}
...
\end{thm}%???notation do not agree with the first coare inequality
...
In this section we assume that homology are taken with the coefficients $\ZZ_2$, but one can play with other coefficients as well.
