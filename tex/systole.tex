...
if not we can homotopy each into a broken geodesic line.

Choose two sequences of points $x_i=\gamma_0(t_i)$ and $y_i=\gamma_1(t_i)$
for a fine partition $0=t_0<t_1<\dots<t_n=1$.
We can assume that each arc $x_{i-1}$ to $x_i$ on $\gamma_0$ and from $y_{i-1}$ to $y_i$ on $\gamma_1$ has length smaller than $\eps$.
Therefore every quadrilateral $x_{i-1}y_{i-1}y_ix_i$ formed by concatenation  of $[x_{i-1}y_{i-1}]$, arc of $\gamma_1$, $[y_ix_i]$, and arc of $\gamma_0$ has length smaller than $s$.
It follows that this curve is contractible.
Applying this observation for each quadrilateral $x_{i-1}y_{i-1}y_ix_i$ we get the statement.
\qeds
...
So, taking \ref{ex:fillrad-inj} into account, we proved the following yet more general theorem.
...
A wide open conjecture says that the optimal constant is $\pi/\sqrt[n]{v_n/2}$ where $v_n$ denotes the volume of $n$-dimensional unit sphere;
this ratio grows as $O(\sqrt n)$.
This is the systole ratio for $n$-dimensional real projective space with canonical metric.
