...
\parit{If part.} Learn about the Kirszbraun theorem and apply it together with closest-point projection.
...
Let $f'\:A\cup\{w\}\to \spc{B}$ be extension of $f$ by $w\mapsto o$; note that $f'$ is short.
...
Let $\{B_\alpha\}$ be an arbitrary collection of closed balls in an injective space.
Argue as in \ref{ex:extr-ball} to show that the intersection $\bigcap_\alpha B_\alpha$ is injective.
...
From above, $\spc{Y}'$ is injective.
Use that $\spc{Z}$ is minimal to show that $s(\spc{Y}')\subset \spc{Y}'$.
Show that $\diam \spc{Y}'\le \tfrac12\cdot\diam \spc{Y}$.

Consider sequence of nested injective spaces $\spc{Y}=\spc{Y}_0\supset \spc{Y}_1\supset\dots$ such that $\spc{Y}_{n+1}=\spc{Y}_{n}'$.
...
For the noncompact case, prove an approimate version of \ref{eq:pfgq+xvwy} and apply it the same way.
