...
\parit{Remark.} The same argument can be used to construct the so-called \index{geodesic bicombing}\emph{geodesic bicombing} on injective space --- a useful tool introduced by Urs Lang \cite[3.6]{lang-2013}.
...
Show and use that the functions in $\Inj\spc{X}$ are 1-Lipschitz and uniformly bounded.
...
\parit{Source:} Suggested by Rostislav Matveyev.
...
that is, for any three points $x,y,z$ there is a \textit{unique} point $m$ (it is called the \index{median}\emph{median} of $x$, $y$, and $z$) that lies on {}\emph{some} geodesics $[xy]$, $[xz]$ and $[yz]$.
...
\parbf{\ref{ex:petrunin-stadler}.} Consider the space $\spc{K}^{\spc{X}}$ of all maps $\spc{X}\z\to \spc{K}$ equipped with the product topology.
...
\parit{Source:} \cite[7.1]{petrunin-stadler}.
