...
Conclude that any hinge in a $\CAT(0)$ or $\CBB(0)$ space has defined angle.
\end{thm}

\begin{thm}{Exercise}\label{ex:contractible}
Fix a point $p$ in a  be a complete length $\CAT(0)$ space~$\spc{X}$.
Given a point $x\in \spc{X}$, denote by $\gamma_x$ a (necessarily unique) geodesic path from $p$ to $x$.
