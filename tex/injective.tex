...
In this lecture we discuss {}\emph{injective spaces} also known as {}\emph{hyperconvex spaces}.
...
We follow closely the paper of John Isbell \cite{isbell}.

\section{Admissible and extremal functions}

Let $\spc{X}$ be a metric space.
A function $r\:\spc{X}\to\RR$ is called \index{admissible function}\emph{admissible} if the following inequality
...
A minimal admissible function will be called \index{extremal function}\emph{extremal}.
...
that is,
\[|r(p)-r(q)|\le \dist{p}{q}{}\]
for any $p,q\in\spc{X}$.\footnote{In other words, any extremal function is an extension function; see the definition on page \pageref{page:extension function}.}
\end{subthm}
...
Moreover if $\spc{X}$, then there is $q$ such that 
...
\parit{\ref{lem:extremal-lipschitz}.}
...
\begin{thm}{Exercise}
Suppose a length space $\spc{W}$ have two subspaces $\spc{X}$ and $\spc{Y}$ such that $\spc{X}\cup\spc{Y}=\spc{W}$ and $\spc{X}\cup\spc{Y}$ is one-point set.
Assume $\spc{X}$ and $\spc{Y}$ are injective.
Show that  $\spc{W}$ is injective
\end{thm}

\begin{thm}{Exercise}
Show that the Urysohn space is {}\emph{finitely hypercovex} but not {}\emph{countably hyperconvex};
