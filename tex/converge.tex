...
respectively and such that $|\spc{X}'-\spc{Y}'|_{\Haus\spc{W}} < r$. 
...
$\iota[\spc{P}]=[\spc{P}]$ for any isometry $\iota\:\GH\to\GH$.
\end{subthm}
...
where $\iota$ is any isometry of $\RR^2$.
\end{thm}
...
the functions $f\:\spc{X}\times \spc{Y}\to\RR$ that can appear this way will be called \emph{appropriate}.
...
Applying the Arzelà--Ascoli theorem, we can get an  appropriate functions $f\:\spc{X}\times\spc{Y}\to\RR$ 
that minimize $\min\{a_f,b_f\}$.
It reamins to apply \ref{obs:GH=min-appropriate}.
\qeds
...
Therefore, $\spc{W}_n$ is a compact $\eps_n$-net of $\spc{Q}$.
