...
In this lecture we cook up a new metric space $\Haus\spc{X}$ from subsets of a given metric spaces $\spc{X}$.
Further, we prove the Blaschke selection theorem;
it says that $\spc{X}$ is compact, then so is  $\Haus\spc{X}$.
...
The \index{support function}\emph{support function} $h_A\colon\mathbb{R}^n\to\mathbb{R}$ of a  non-empty closed set $A\subset \RR^n$ is defined as 
\[h_A(x)
\df
...
In this lecture we introduce the so called Gromov--Hausdorff metric on the isometry classes of compact metric spaces.
Further, we prove the Gromov selection theorem that characterize compact subsets in the obtained space.
