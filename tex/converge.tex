...
A space $\spc{X}$ is called \index{doubling space}\emph{doubling} if it is $M$-doubling for some $M$.
...
Often the isometry class of the limit can be fixed by marking a point $p_n$ in each space $\spc{X}_n$, it is called \index{pointed convergence}\emph{pointed Gromov--Hausdorff convergence} --- we say that $(\spc{X}_n,p_n)$ converges to $(\spc{X}_\infty,p_\infty)$ if there is a proper metric space $\bm{X}$ as in \ref{propery:GH} such that $\spc{X}_n\Hto \spc{X}_\infty$ and $p_n\to p_\infty$.
...
The following theorem is an analog of Gromov's selection theorem for this convergence; the proof is nearly the same.
...
The space $\spc{X}^\theta$ is called \index{snowflake}\emph{snowflake} of~$\spc{X}$.
