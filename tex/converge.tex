...
Then $|A-B|_{\Haus\spc{X}}< R$ if and only if if and only if 
...
Show that closure of the union $K_1\cup K_2\cup\dots$ is compact.
...
Note that  $B$ is finite set in $\Haus\spc{X}$.
...
Among the plane figures bounded by closed curves of length at most $\ell$ the round disk has maximal area.
\end{thm}
...
Couple of years later an equivalent definition was given by Wilhelm Blaschke~\cite{blaschke}.
...
Note that passing to the subspace $\spc{X}'\cup\spc{Y}'$ of $\spc{Z}$ does not effect the definition.
...
A relation $\approx$ between points in $\spc{X}$ and $\spc{Y}$ is called $\eps$-approximation if 
...
The same argument produce a distance-preserving map $g_\infty\:\spc{Y}\z\to \spc{X}$.
If $f_\infty$ is not surjective, then neither is the composition $f_\infty\circ g_\infty\:\spc{Y}\to \spc{Y}$.
...
Let $\spc{Q}$ be a family of (isometry classes) of compact metric spaces.
...
Then we say that $\spc{Q}$ is \index{uniformly totally bonded set}\emph{uniformly totally bonded}.
\end{thm}

Suppose family $\spc{Q}$ and sequence $(\eps_n)$ as in the definition.
Observe that in this case $\diam\spc{X}<\eps_1$ for any  $\spc{X}$ in $\spc{Q}$; that is, {}\emph{spaces in a uniformly totally bonded set have uniformly bounded diameter}.
...
It remains to observe that $\spc{X}_\infty$ is the Gromov--Hausdoff limit of $(\spc{X}_n)$.
\qeds
...
in particular, it is is separable.
