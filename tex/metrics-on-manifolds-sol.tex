...
Christopher Croke conjectured that the optimal bound is $\tfrac4\pi$ and the round sphere is the only space that achieves this \cite[Conjecture 0.3 in][]{croke} --- if you solved this part of problem, then publish.
...
To solve the second part of problem, start with $g_\infty$ and construct $g_n$ by  adding many tiny bubbles.
The volume can be increased arbitrary with arbitrary small change of metric.
...
The space $\spc{K}$ is called an \index{K(G,1) space}\emph{Eilenberg--MacLane space of type $K(G,1)$}, or briefly a $K(G,1)$ space.
