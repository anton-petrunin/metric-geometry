...
\parbf{\ref{ex:huas/vol}.}

\parbf{\ref{ex:2top-discs}.}
...
Similarly for each vertex $v_{ijk}$ choose a point $p_{ijk}\in V_i\cap V_j\cap V_k$ and set $f(v_{ijk})=p_{ijk}$.
...
\parit{Remark.} Note that we only used that there is a map $N\#M\to M$ of degree 1. If essential manifold is defined using homologies with integer coefficients, then existence of map of nonzero degree is sufficient.
