...
Use \ref{SHORT.ex:d_GH-and-diam:point} and \ref{SHORT.ex:d_GH-and-diam:scale} to show that the isometry class of $\spc{P}$ is completely determined by the following property
\[|\spc{X}-\spc{Y}|_{\GH} \le \max\{\,|\spc{P}-\spc{X}|_{\GH},|\spc{P}-\spc{Y}|_{\GH}\,\}.\]
for any $\spc{X}$ and $\spc{Y}$.

\parit{Remark.}
In fact, the isometry group of space $\GH$ is trivial.
