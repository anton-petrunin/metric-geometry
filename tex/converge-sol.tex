...
It can be used to construct a more interesting examples for \ref{SHORT.ex:Hausdorff-bry:bry}.
...
\parit{Remark.}
In fact $|\spc{A}_r-\spc{B}_r|_{\GH}=\tfrac12$ for all large $r$; try to show it.
...
So, if we define fundamental group as the inverse image of groups of deck transformations for all the coverings of the given space, then one may say that Gromov--Hausdorff limit of simply connected length spaces is simply connected.
...
By Jordan curve theorem, $\gamma$ divides $\mathbb{S}^2$ into two disks, say $D_1$ and $D_2$.
...
then we can pass to a subsequence such that $\dist{\spc{X}_n}{\spc{X}_\infty}{\GH}\ge\eps$ for some $\eps>0$.
