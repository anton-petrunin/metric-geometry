...
\parit{Remark.}
There are the so-called {}\emph{lakes of Wada} --- an example of three (and more) open topological disks in the plane that have identical boundaries.
...
I~do not know a good description of spaces that admit such a retraction.
For example, the Euclidean plane does not satisfy this condition \cite{petrunin-2022-MO-center, przeslawski-yost}.
%It might happen that all such complete length spaces are injective.???
...
$a\cdot\spc{X}\z\hookrightarrow\spc{Z}$,
$b\cdot\spc{X}\z\hookrightarrow\spc{Z}$ are distance-preserving.
...
On the other hand, any curve from $p_1$ to $p_2$ must cross $\gamma$, so its length is about 2 --- a contradiction.
