...
forms a \index{semimetric space}\emph{semimetric space};
...
\ref{eq:dist-in-T_p}
defines a semimetric on $\mathfrak{T}_p$.
...
Show that for any point $p\in \spc{X}$  the tangent space $\T_p\spc{X}$ is isometric to a subset of the ultra-tangent space $\T_p^\omega\spc{X}$ (defined in \ref{sec:tan+asymptotic}).
...
Suspension is a spherical analog of cone construction (\ref{sec:Products and cones}).
...
Then by \ref{thm:PL-CAT},
$\Link_\sigma\spc{P}$ is $\CAT(1)$ for every simplex $\sigma$ 
in~$\tau$. 

Each of these links is an all-right spherical complex and by \ref{thm:PL-CAT}, none of these links can contain a geodesic circle shorter than $2\cdot\pi$. 

Therefore Proposition~\ref{prop:no-trig} and \ref{clm:3pi/2} 
imply the ``only if'' part.

\parit{If part.} 
By \ref{obs:link-of-flag} and \ref{thm:PL-CAT},
