...
This lecture is based on the discussion of Urysohn space given by Mikhael Gromov \cite{gromov-2007}.
...
\parit{Proof.} It is sufficient to show that $\iota$ is distance-nondecreasing and distance-nonexpanding, or \index{short map}\emph{short}.

By the triangle inequality 
\[|a_n(x)-a_n(y)|\le \dist{x}{y}{\spc{X}}.\]
Therefore $\iota$ is \index{short map}\emph{short}.
...
If instead of extension functions we consider only $d$-extension functions and assume in addition that $\diam \spc{U}\le d$, then we arrive to a definition of {}\emph{$d$-extension property}.
...
By construction, there is a point $p\in \spc{X}_{n+1}$ that meets the condition in Definiton~\ref{def:finite+1}.
...
So, according to \ref{ex:frechet}, $\ell^\infty$ is a universal space. 
The following proposition shows that an Urysohn space is univeral as well.
Since an Urysohn space is separable, it might be considered as a better 
universal space.
