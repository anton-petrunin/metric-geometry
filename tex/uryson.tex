...
The functions $\bar f$ and $\bar f_d$ in the above exercise are called \index{Katětov extensions}\emph{Katětov extensions} of $f$ and $\spc{X}$ is called its {support of extension function}\emph{support}, briefly \index{$\supp$}$\supp \bar f=\spc{X}$.
...
Start with $\spc{X}_0=\spc{X}$ and construct a nested sequence of spaces $\spc{X}_0\subset\spc{X}_1 \subset{}\dots$ as at the very end of proof of \ref{prop:univeral-separable}.
Note that 
...
From above, for any isometry of $\spc{X}$ can be extended to an isometries of $\spc{X}'$, $\spc{X}''$ and so on.
...
The following construction uses the idea of \ref{prop:sep-in-urys}, but it is applied {}\emph{back-and-forth} to ensure that the obtained distance-preserving map is onto.
...
The following exercise answers a question posed by Pavel Urysohn \cite[§$2(6)$]{urysohn}.
...
The \index{cut metric}\emph{cut metric} $\delta_S$ on $X$ is a pseudometric such that $\delta_S(x, y) = 1$ if $x$ and $y$ are separated
...
Recall that vertex set of any graph $\Gamma$ comes with a metric ---
the distance between two vertices is minimal number of edges in a path connecting them.
