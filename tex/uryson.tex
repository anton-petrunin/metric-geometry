...
Denote by \index{$\ell^\infty(\spc{X})$}$\ell^\infty(\spc{X})$ the space of all bounded functions on $\spc{X}$ equipped with sup-norm.
...
The following two exercises show that in this respect $\ell^\infty$ is very different from \index{$\ell^1$}$\ell^1$;
