...
\begin{thm}{Exercise}
Let $\spc{X}$ be a metric space with $\diam\spc{X}\le d$.
Denote by $\spc{X}^d$ the space of all $d$-extension functions on $\spc{X}$ equipped with the metric defined by the sup-norm.
Show that 

\begin{subthm}{}
$x\mapsto \distfun_x$ defines a distance preserving inclusion $\spc{X} \hookrightarrow\spc{X}^d$, and
\end{subthm}

\begin{subthm}{}
any isometry $f\: \spc{X}\to \spc{X}$ can be extended to a unique isometry $f^d\: \spc{X}^d\to \spc{X}^d$.
\end{subthm}
...
The Urysohn space is homeomorphic to the countable product of the real lines;
it was proved by Vladimir Uspenskij \cite{uspenskij}.

It turns out that \textit{there are noncomplete finite-set homogeneous metric space that meets the extension property}. 
This question was originally posted by Pavel Urysohn \cite[§$2(6)$]{urysohn} and 
anwered by Miroslav Katětov \cite{katetov};
long after his example, it was again mentioned as an open question \cite[p. 83]{gromov-2007}.

\parit{Sketch of Katětov's construction.}
Note that we essentially produced a construction of a $d$-Urysohn space from a given separable metric space $\spc{X}$ with diameter~$d$.
Denote it by $\spc{U}_d(\spc{X})$, note that $\spc{X}$ can be considered as a subspace of $\spc{U}_d(\spc{X})$.

This construction can be applied recursively; it produces a nested sequence of Urysohn spaces $\spc{X}_1\subset\spc{X}_2\subset\dots$ such that $\spc{X}_{n+1}=\spc{U}_d(\spc{X}_n)$ for any $n$.
The union $\bigcup_n\spc{X}_n$ is the needed example.
\qeds
