...
any isometry $f\: \spc{X}\to \spc{X}$ can be extended to a unique isometry $F\: \spc{X}^\infty\to \spc{X}^\infty$.

Note that for arbitrary separable space $\spc{X}=\spc{X}_0$ one can construct a nested sequence of spaces 
\[\spc{X}_0\subset \spc{X}_1\subset\dots\]
such that $\spc{X}_{n+1}=\spc{X}_{n}^\infty$.
It is easy to modify the proofs of \ref{prop:univeral-separable} and \ref{prop:completion-univeral} to show that of the completion of the union $\bigcup_n\spc{X}_{n}$ is an Urysohn space $\spc{U}$ that comes with a distance-preserving inclusion $\spc{X}\hookrightarrow \spc{U}$.
\qeds
