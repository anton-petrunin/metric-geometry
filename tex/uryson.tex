...
Often we do not distinguish between normed space and the corresponding metric space.\footnote{By Mazur--Ulam theorem, the metric remembers the affine structure of the space; a slick proof of this theorem was given by Jussi V\"{a}is\"{a}l\"{a} \cite{vaisala}.}
...
Since $\spc{U}$ has extension property, there is a point $s_i'\in \spc{U}$ such that
\[\dist{s_i'}{s_j'}{\spc{U}}=\dist{s_i}{s_j}{\spc{S}}\]
for any $j<i$.

The constructed map $s_i\mapsto s_i'$ is  distance-preserving map; it remains to extend it to a distance-preserving map on whole $\spc{S}$.
\qeds
...
The following construction uses the same idea as in the proof of \ref{prop:sep-in-urys}, but we need to apply it {}\emph{back-and-forth} to ensure that the constructed distance-preserving map is onto.
While \ref{prop:sep-in-urys} implies that there are distance-preserving maps $\spc{U}\to \spc{U}'$ and $\spc{U}'\to \spc{U}$,
it does not solely imply the existence of an isometry.
...
This is a question of Pavel Urysohn; it appeared already in \cite[§2(6)]{urysohn}
...
so $\spc{U}_\infty$ the Urysohn space.
...
Which of the following metric spaces
