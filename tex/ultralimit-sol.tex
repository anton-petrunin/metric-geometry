...
Consider the vertex $\omega=\lim_{n\to\omega}n$ in the ultrapower $\spc{T}^\omega$.
Observe that $\omega$ has infinite degree.
...
Show that there is $\delta>0$ such that sides of any geodesic triangle intersect a disc of radius $\delta$.
Conclude that any geodesic triangle in $\Asym\spc{L}$ is a tripod.
Make a conclusion.

\parit{\ref{SHORT.ex:Asym(Lob):homogeneous}.} Observe that $L$ is homogeneous and use it.

\parit{\ref{SHORT.ex:Asym(Lob):continuum}.} 
By \ref{SHORT.ex:Asym(Lob):others}, it is sufficient to show that $p_\omega$ has a continuum degree.
...
The 3-regular tree is not homogeneous, but it is vertex-homogeneous; the latter is sufficient to prove \ref{SHORT.ex:Asym(Lob):homogeneous}.
No changes needed in \ref{SHORT.ex:Asym(Lob):continuum}.

\parit{Remark.}
As it shown in \cite{dyubina-polterovich}, the properties \ref{SHORT.ex:Asym(Lob):homogeneous} and \ref{SHORT.ex:Asym(Lob):continuum} describe the tree $\spc{T}$ up to isometry.
