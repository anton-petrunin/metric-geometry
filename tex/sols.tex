...
\begin{align*}
&\dist{\gamma_x(t)}{\gamma_z(\tau)}{}\le 
\\
&\quad\le 
\ \dist{\gamma_x(t)}{\gamma_y(s)}{}+\dist{\gamma_y(s)}{\gamma_z(\tau)}{}<
\\
&\quad<{t^2+s^2-2\cdot t\cdot  s\cdot \cos(\alpha+\eps)}\ +
\\
&\qquad+\sqrt{s^2+\tau^2-2\cdot s\cdot \tau\cdot \cos(\beta+\eps)}\le
\end{align*}

Below we define 
$s(t,\tau)$ so that for 
$s\z=s(t,\tau)$, this chain of inequalities can be continued as follows:
\[\le
...
Consider two model triangles $[\tilde x\tilde y\tilde p]\z=\modtrig(xyp)$ and $[\tilde x\tilde y\tilde q]=\modtrig(xyq)$ such that $\tilde p$ and $\tilde q$ lie on the opposite sides of the line $\tilde x\tilde y$.
...
Use \ref{ex:noncreasing-CAT}, to show that the map $(t,x)\z\mapsto \gamma_x(t)$ is continuous; that is $h_t(x)=\gamma_x(t)$ defines a homotopy.
...
Since $f$ is concave,
$f(\bar z)<f(z_0)$ for any $\bar z \z\in \left[zz_0\right[$.
...
consider the map $(a,b)\z\mapsto m(p,\gamma(a),\gamma(b))$ of the triangle $\triangle$ defined by $0\z\le a \z\le b\z\le 1$.
