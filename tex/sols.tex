...
Denote by $\spc{L}=\Conv\spc{K}$ the minimal convex closed set in $\ell^\infty$ that contains $\spc{K}$.
Observe that $\spc{L}$ is a length space.

It remains to show that since $\spc{K}$, so is $\spc{L}$ --- do it.
...
\[\dist{(\ell_n)}{(m_n)}{\spc{Y}}=\tfrac1{2^{n-1}},\]
where $n$ is minimal index such that $\ell_n\ne m_n$.

It remains to observe that $\spc{Y}$ is compact and any space $\spc{X}$ in $\spc{Q}$ admits a distance non-decreasing map $\spc{X}\to \spc{Y}$.
...
\parbf{Exercise~\ref{ex:GH-SC:nonsc-limit}.}
...
\parbf{Exercise~\ref{ex:sphere-to-ball:3}.}
...
Sourse: \cite[Ex. 7.5.17]{burago-burago-ivanov}. 
