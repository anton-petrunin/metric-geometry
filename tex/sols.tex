...
Assume that no triangle condition does not hold at a vertex $v$;
that is $v$ is a vertex of there edges $e_x$, $e_y$, and $e_z$;
each pair of edges belong to one square, but all three of them do not belong to one cube.
It is sufficient to consider only this case --- for higher dimensional cube we have the same configuration multiplied by a cube.
...
so we have a configuration of three squares, three edges and one common vertex as above.
Observe that the centers of the squares $x$, $y$, and $z$ do not have a median; that is, $(Q,\ell^1)$ is not median.
