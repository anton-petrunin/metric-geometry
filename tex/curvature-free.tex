...
\begin{itemize}
\item For real projective plane $\RP^2$ the constant is $\tfrac\pi2$ --- the equality holds for a quotient of a round sphere by isometric involution \cite{pu}.
...
Systolic intequality holds for the 3-dimensional torus $\TT^3$. 
\end{thm}
...
Suppose that $\spc{M}$ is a closed $n$-dimensional Reimannian manifold with injectivity radius $r$; that is, if $\dist{p}{q}{\spc{M}}<r$, then there is geodesic $[pq]_{\spc{M}}$ is uniquely defined.
Show that
