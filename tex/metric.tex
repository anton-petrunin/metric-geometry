...
\end{enumerate}
A set $\spc{X}$ with a metric on it is called \emph{metric space};
most of the time we keep the same notation for the metric space and its underlying set.
\end{thm}

\begin{thm}{Exercise}
Let $p$, $q$, $x$, and $y$ be points in a metric space $\spc{X}$.
Show that
...
A subset $N$ of a metric space $\spc{K}$ is called \emph{$\eps$-net} if any other point $x$ lies on the distance less than $\eps$ from a point in $N$.
...
If $\spc{X}$ is an $\infty$-metric space, then we assume that $x$ and $y$ lie in one metric component; that is, $\dist{x}{y}{\spc{X}}<\infty$.
