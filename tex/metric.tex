...
Alternatively totally bounded spaces can be defined the following way.

A metric space $\spc{K}$ is \emph{totally bounded} if for any $\eps>0$ there is a finite \emph{$\eps$-net}; that is, a finite set of points $x_1,\dots,x_n\in \spc{K}$ such that any other point $x$ lies on the distance less than $\eps$ from one of $x_i$.
...
Show that any compact metric space is isometric $\spc{K}$ to a subspace of a compact geodesic space. 
\end{thm}
