...
Consider all $\infty$-semimetrics $|\ -\ |_\alpha$ on $\spc{X}/\sim$ such that the maps $\spc{X}\to \spc{X}/\sim$ defined by $x\mapsto [x]$ are \index{short map}\emph{short};
that is,
...
A \index{geodesic path}\emph{geodesic path} is a geodesic with constant-speed parametrization by the unit interval $[0,1]$.
...
\begin{thm}{Exercise}\label{ex:HR}
Let $\spc{X}$ be a locally compact geodesic space.
Suppose that any geodesic $\gamma\:(a,b)\to\spc{X}$ can be extended to $[a,b]$.
Show that $\spc{X}$ is proper.
\end{thm}


\begin{thm}{Advanced exercise}\label{ex:gross}
Show that for any compact connected metric space $\spc{X}$ there is a number $\ell$ such that for any finite collection of points there is a point $z$ that lies on average distance $\ell$ from the collection;
