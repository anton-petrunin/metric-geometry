...
\parbf{Maximal metric and gluing.}
Let $\{\,\dist{\ }{\ }{\alpha}\,\}$ be a family of $\infty$-semimetrics on a fixed set.
Observe that
\[\dist{x}{y}{}\df\sup_\alpha\{\,\dist{x}{y}{\alpha}\,\}\]
defines an $\infty$-semimetric; it is called \index{maximal metric}\emph{maximal metric} of the family.

Suppose $\spc{X}$ and $\spc{Y}$ are metric spaces and $\sim$ is an equivalence relation on the disjoint union $\spc{X}\sqcup\spc{Y}$.
Given $x\in\spc{X}\sqcup\spc{Y}$, 
denote by $[x]$ its equivalence class in the quotient $\spc{X}\sqcup\spc{Y}/\sim$.
Consider all $\infty$-semimetrics $|\ -\ |_\alpha$ on $\spc{X}\sqcup\spc{Y}/\sim$ such that  both maps $\spc{X},\spc{Y}\to \spc{X}\sqcup\spc{Y}/\sim$ defined by $x\mapsto [x]$ are \index{short map}\emph{short};
that is,
\[|[x]-[x']|_\alpha\le |x-x'|_{\spc{X}}
\quad\text{and}\quad
|[x]-[x']|_\alpha\le |x-x'|_{\spc{X}}\]
for any $x,x'\in \spc{X}$ and $y,y\in \spc{Y}$.
Let us equip $\spc{X}\sqcup\spc{Y}/\sim$ with the maximal metric of this family; in general it is a $\infty$-semimetric.
The space $\spc{Z}$ that corresponds to the obtained $\infty$-semimetric space is called \index{gluing}\emph{gluing} of $\spc{X}$ and $\spc{Y}$ along $\sim$.
Similarly one can define gluing of arbitrary collection of metric spaces.
...
The following exercise shows that the topology of the described metric gluing might deffer from the corresponding topological gluing.
