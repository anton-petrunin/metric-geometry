...
where the greatest lower bound is taken for all paths $\alpha$ from $x$ to $y$ in $A$.\footnote{Note that while this notation slightly conflicts with the previously defined notation for distance on a general metric space, we will usually work with ambient length spaces where the meaning will be unambiguous.}
...
\begin{enumerate}[(a)]
\item $|v|\ge 0$ for any $v\in \spc{V}$;
...
Suppose $\spc{X}$ is separable; that is, it contains a countable, dense set $\{w_n\}$.
Then for any point $x_0\in \spc{X}$, the map $\iota\:\spc{X}\to \ell^\infty$ defied by 
\[\iota\:x\mapsto (\dist{w_1}{x}{\spc{X}}-\dist{w_1}{x_0}{\spc{X}}, \dist{w_2}{x}{\spc{X}}-\dist{w_2}{x_0}{\spc{X}},\dots)\]
is a distance preserving embedding.
\end{thm}

\parit{Proof.}
By the triangle inequality 
\[|\dist{w_n}{x}{\spc{X}}-\dist{w_n}{x_0}{\spc{X}}|\le\dist{x}{x_0}{\spc{X}}.\]
Therefore the sequence $\ell_n=\dist{w_n}{x}{\spc{X}}-\dist{w_n}{x_0}{\spc{X}}$ is bounded; that is $\iota(x)\in\ell^\infty$ for any $x$.

Again by the triangle inequality 
\[|\dist{w_n}{x}{\spc{X}}-\dist{w_n}{y}{\spc{X}}|\le \dist{x}{y}{\spc{X}}.\]
Therefore $\iota$ is short.

Again by triangle inequality we have 
\[|\dist{w_n}{x}{\spc{X}}-\dist{w_n}{y}{\spc{X}}|\ge \dist{x}{y}{\spc{X}}-2\cdot\dist{w_n}{x}{\spc{X}}.\]
Since the set $\{w_i\}$ is dense, we can choose $w_n$ arbitrary close to $x$.
Whence the value $|\dist{w_n}{x}{\spc{X}}-\dist{w_n}{y}{\spc{X}}|$ can be chosen arbitrary close to $\dist{x}{y}{\spc{X}}$.
In other words 
\[\sup_n\{\,|\dist{w_n}{x}{\spc{X}}-\dist{w_n}{y}{\spc{X}}|\,\}\ge \dist{x}{y}{\spc{X}};\]
hence $\iota$ is distance-preserving.
\qeds

\parbf{Remark.}
Note that if the space $\spc{X}$ is bounded (that is $\diam\spc{X}<\infty$)
then 
\[x\mapsto (\dist{w_1}{x}{\spc{X}}, \dist{w_2}{x}{\spc{X}},\dots)\]
is also distance preserving embedding $\spc{X}\to \ell^\infty$.
...
Namely he made the following claim; its proof is left as an exercise.
