...
The corresponding metric space $\bar{\spc{X}}$ is called \index{completion}\emph{completion} of $\spc{X}$.
...
More generally, a subset $N$ is called \index{net}\emph{$\eps$-net} of a subset $S\subset\spc{K}$ if any point $x\in S$ lies on the distance less than $\eps$ from a point in $N$. 
