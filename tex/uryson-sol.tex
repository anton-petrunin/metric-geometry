...
\begin{itemize}
\item a surjective continuous map $f\:\spc{K}\to \spc{M}$ for any compact metric space $\spc{M}$, and
\item a distance-preserving (linear) map
\[\spc{C}(\spc{K})\to \spc{C}([0,1]).\]
\end{itemize}

\parit{(a).}
Choose a surjective continuous map $f\:\spc{K}\to \spc{X}$.
Map $\spc{X}\z\to \spc{C}(\spc{K})$ by $x\mapsto \distfun_x\circ f$.

\parit{(b).}
Choose a dense set $\{x_0,x_1,\dots\}\subset \spc{X}$.
...
Choose a continuous surjective map $\spc{K}\to \spc{L}$;
let us denote it by $a\mapsto h_a$.
Consider the map $\spc{X}\to \spc{C}(\spc{K})$ that sends $x\in \spc{X}$ to the function
$a\mapsto h_a(x)$.

\parit{Comments.}
This statement is due to Stefan Banach \cite[XI §8, Theorem 10]{banach-1932};
a complete proof is given in the lecture notes of Juha Heinonen \cite[3.6]{heinonen-2003}, which are well worth reading.
